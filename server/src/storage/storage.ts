// import { users, budgets, expenses, expenseCategories, savingsGoals, type User, type InsertUser, type Budget, type InsertBudget, type Expense, type InsertExpense, type ExpenseCategory, type InsertExpenseCategory, type SavingsGoal, type InsertSavingsGoal } from "../models/schema.js";
...
//     return await db.select().from(savingsGoals).where(eq(savingsGoals.user_id, userId));
//   }
// }
...
//   async getSavingsGoalsByUserId(userId: number): Promise<SavingsGoal[]> {
//     return Array.from(this.savingsGoals.values()).filter(goal => goal.user_id === userId);
//   }
// }
